...
what turns a cluster into a finding, and the paper's topic tables print your
labels, not the keywords.

> **Where the labels actually surface.** Only `07_rq1_gamification.py` reads
> `manual_label`, and only from `topic_info_rq1_tracker.csv`. The RQ4 and RQ5
> notebooks do **not** load their sub-model sheets, and no notebook loads the
> 40-topic sheet. So the labels are for *you* — the topic tables and the
> qualitative passages you write by hand. Do not expect them to appear in
> notebook output, and do not read a notebook's silence as a missing label.
...
> ⚠️ **This sheet is regenerated by two different things** — `06_models.py`
> (M2/RQ1) and `07_rq1_gamification.py` section 6. Both now go through
> `write_coding_sheet`, which refuses to overwrite a sheet that already has
> `theme_code` filled in and logs a warning instead. Your coding is safe from a
> re-run, but if you ever *want* a fresh sheet you must delete the file first.
...
### ⚠️ What the coded 50 actually are — read before writing RQ1

**All 50 come from a single app** (*Athan: Prayer Times & Al Quran*). This is a
sampling artefact, not a property of the corpus. `06_models.py` selects them
with `d.nsmallest(50, "sent_num")`, and `sent_num` is three-valued (−1/0/+1), so
every negative review ties at −1 and pandas returns the first 50 **in frame
order**, which is app order. One app therefore fills the whole sheet.

The coding is still good evidence — it just is not evidence about the ecosystem.
Two honest ways to use it:

- **Report it as a single-app case study.** Say plainly that the qualitative
  codes come from the largest tracker app and are illustrative of that app, not
  a cross-app sample. Costs nothing and is defensible.
- **Re-draw stratified across apps and re-code.** Another hour, and the codes
  would then support a cross-app claim.

Given the deadline, the first is reasonable — but it **must** be stated in
Methods and in the RQ1 passage. Do not describe these 50 as a corpus-wide
sample.

Two smaller things about the same sheet:

- **18 of the 50 carry 4–5 stars.** Selection is by RoBERTa sentiment, not star
  rating, so the set is "reviews whose text reads negative", which includes
  otherwise-positive reviews carrying one specific complaint. That is fine — say
  "reviews expressing tracker dissatisfaction", not "negative reviews".
- **The consolidation pass is still open.** There are 33 distinct codes across
  50 reviews, so the codes are still close to one-per-review. Merge them into a
  handful of themes before writing; the raw 33 are not a result.

The codes do already carry RQ1's headline, and it is a strong one: the dominant
theme is **logging integrity** — redesign breaking the prayer log, backfill
windows, lost history, counts regressing — roughly 19 of 50. Guilt or judgment
appears in **2 of 50**. That independently corroborates the statistical finding
that guilt is under-represented, from a completely different direction.
...
are the costliest complaint in the corpus. `madhab` flips sign in the ordinal
check (β = +0.22, p = 0.65). See [`PAPER_DISCLOSURES.md`](PAPER_DISCLOSURES.md)
§4.3.
